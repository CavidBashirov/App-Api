...
namespace Service.Services
{
    public class AccountService : IAccountService
    {
        ...
        {
            AppUser user = await _userManager.FindByIdAsync(request.UserId) ?? throw new NotFoundException("User not found");
            IdentityRole role = await _roleManager.FindByIdAsync(request.RoleId) ?? throw new NotFoundException("Role not found");
        ...
        public async Task<LoginResponse> SignInAsync(LoginDto request)
        {
            ...
            string token = _tokenService.GenerateJwtToken(existUser,(List<string>)userRoles);
